...
export default function MenuTabFields({ formData, setFormData }: { formData: Article, setFormData: any }) {
  const [isMenuTab, setIsMenuTab] = useState(!!formData.tab); // Replace with actual state to track if Menu Tab is checked
  const [menuTabName, setMenuTabName] = useState(formData.tab?.name || ''); // Replace with actual state for Menu Tab Name
  const [menuTabLink, setMenuTabLink] = useState(formData.tab?.link || ''); // Replace with actual state for Menu Tab Link
  return (
      <div className="mb-4">
        ...
        <label className="flex items-center space-x-2">
          <input
            ...
            onChange={() => {
              ...
              if (!isMenuTab) {
                setFormData({ ...formData, tab: { name: menuTabName, link: menuTabLink } });
              } else {
        ...
        {isMenuTab && (
          ...
          <div className="mt-2">
            ...
            <input
              ...
              id="menuTabName"
              value={menuTabName}
              onChange={(e) => { setMenuTabName(e.target.value); setFormData({ ...formData, tab: { name: e.target.value, link: menuTabLink } }); }}
          ...
          <div className="mt-2" hidden={formData !== undefined} >
            ...
            <input
              ...
              id="menuTabLink"
              value={menuTabLink}
              onChange={(e) => { setMenuTabLink(e.target.value); setFormData({ ...formData, tab: { name: menuTabName, link: e.target.value } }); }}
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          </>
